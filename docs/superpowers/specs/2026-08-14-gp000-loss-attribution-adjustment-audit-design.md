...
### Sonnet Important 整改边界

本轮只允许修改专项审计脚本、专项测试、设计文档、风险报告和治理台账。不得修改
`helix/` 下的归因、标签、回测或因子计算实现，不得重新运行全量审计生成新的数值结论。
既有报告数值作为已冻结证据；代码改动仅增加校验合同、结构化日志和报告渲染约束。

治理台账 D10 与专项报告必须双向引用：D10 记录审计状态、影响量级和报告链接，专项报告
明确引用 D10 条目。任何后续 D10 结论变更都必须同时更新两处。

### 复权校验可执行映射

| 设计承诺 | 可执行逻辑 | 单元测试/断言 | 审计日志节点 |
|---|---|---|---|
...
| 五分位方向与单调性 | 日内先分组再过滤未来结果，输出 Q1-Q5 和 Spearman | 验证分组计数、缺失结果留在原组、Q5-Q1 与 Spearman | `quintiles` |
| 报告根因标注无歧义 | 每条根因必须同时包含“是否核心”和“主导亏损” | 缺任一列拒绝发布；报告测试锁定目标错配为主导、复权问题非主导 | `root_causes` |

结构化日志在证据计算完成后由纯函数从同一 `evidence` payload 生成，每个节点输出一条
JSON 记录。日志不得重新计算统计量，确保报告、机器产物和终端输出使用同一证据源。
