...
def test_prepare_rebuilds_a_panel_with_partial_lineage_metadata(monkeypatch, tmp_path):
    shape = (2, 1)
    dates = np.asarray(["20240101", "20240102"])
    codes = np.asarray(["000001.SZ"])
    panel_path = tmp_path / "cache" / "panel.npz"
    ...
    fields = {
        ...
        **{field: np.ones(shape) for field in pipeline.PANEL_CACHE_REQUIRED_FIELDS},
        **{field: np.ones(shape) for field in pipeline.PANEL_CACHE_REQUIRED_ADJUSTED_FIELDS},
        "__lineage__open_hfq__source_date": dates,
    }
    np.savez_compressed(panel_path, **fields)
    np.savez_compressed(panel_path.parent / "base_fields.npz", stale=np.zeros(shape))

    fresh = Panel(
        dates, codes, fields={"open": np.ones(shape), "limit_price_observed": np.ones(shape)}
    )
    lineage = make_hfq_lineage(dates, VERSION)
    for field in pipeline.PANEL_CACHE_REQUIRED_ADJUSTED_FIELDS:
    ...
    monkeypatch.setattr(
        pipeline,
        "compute_base_fields",
        lambda panel: field_rebuilds.append(True) or {"fresh": np.ones(panel.shape)},
    )
    monkeypatch.setattr(
        pipeline, "build_universe", lambda panel, store, cfg: np.ones(panel.shape, dtype=bool)
    )
    monkeypatch.setattr(
        pipeline, "build_touch_label", lambda panel, universe, cfg: SimpleNamespace()
    )

    prepared = pipeline.prepare(Config(data=DataConfig(root=tmp_path)))

    assert rebuilds == [True]
    assert field_rebuilds == [True]
    assert set(prepared.fields) == {"fresh"}
