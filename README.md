...
表里没有 D0 收盘价，涨停价靠 `close[D0] = open[D+1] / (1 + label_open_gap)` 反推，再按板块乘 10%/20%/30%（`scripts/fillability.py`）。反推本身过了两道独立校验：

| 校验 | 结果 |
|---|---|
| `close[D0]` 由 gap 推 vs 由 `label_d1_pct_chg` 推 | 中位差 2.8e-7，p99 9.2e-7 |
